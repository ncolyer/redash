...
function transformRecentResponse(data) {
  if (data.results) {
    ...
    const sortedUserRecentDashboards = userRecentDashboards.sort((dashboardA, dashboardB) => {
      const priorityOfDashboardA = _.find(
        parsedRecentDashboards,
        dashboard => dashboard.id === dashboardA.id.toString()
      ).priority;
      const priorityOfDashboardB = _.find(
        parsedRecentDashboards,
        dashboard => dashboard.id === dashboardB.id.toString()
      ).priority;
